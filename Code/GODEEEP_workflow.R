...
if(print_diagnostics){
  ...
  g <- ggplot(data=acs_after_state_tax %>% filter(state=="New.York"),aes(color=Quintile))+
    ...
    ggtitle("Income shares for California by quintile")
...
PC_model(state_level_deciles,grouping_variables = c("state","year"),
         value_col = "shares",id_col = c("state")) %>% mutate(category=Category)%>% compute_gini_deciles(inc_col = "pred_shares",grouping_variables = c("state","year")) %>%
...
national_decile_gdp <- read.csv("state_decile_gdp.csv", stringsAsFactors = FALSE) %>%
