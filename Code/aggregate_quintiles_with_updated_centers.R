...
#Calculate variables we need for PCA
data_for_pc <- Wider_data  %>% 
               ...
               group_by(year) %>% 
               mutate(mean_q1= mean(q1),
                      mean_q2= mean(q2),
                      mean_q3= mean(q3),
                      mean_q4= mean(q4),
                      mean_q5 = mean(q5)) %>% 
               ungroup() %>% 
               mutate(q1 = q1-mean_q1,
                      q2 = q2-mean_q2,
                      q3 = q3-mean_q3,
                      q4 = q4-mean_q4,
                      q5 = q5-mean_q5)

pc_results <- prcomp(data_for_pc[4:8], center = FALSE)
pc_data <- as.data.frame(pc_results$x)
data_for_pc$Component1 <- pc_data$PC1

g <- ggplot(data = data_for_pc, aes(x=gini, y=Component1))+
     geom_point()+
     ggtitle("Component calculated using country wise centers vs gini")
...
GINI_data <- read.csv("C:/Users/nara836/Downloads/WIDER_GINI_data.csv", stringsAsFactors = FALSE) %>% 
             select(iso, year, gini_reported, gdp_ppp_pc_usd2011, population)   
...
final_data <- bind_rows(population_no_adjustment, population_new_entries, population_adjusted_for_shortfall) %>% 
  ...
  mutate(gdp_pcap_quintile = sum(gdp_quintile)/sum(population),
   ...
   ungroup() %>%
    ...
    dplyr::select(GCAM_region_ID, year, category, shares, gdp_pcap) %>%
    distinct() %>% compute_gini_GCAM_region_ID() %>% group_by(GCAM_region_ID) %>% 
    mutate(mean_gini= last(gini)) %>% ungroup()

final_data %>% 
  select(GCAM_region_ID,year,category,shares) %>% 
  ...
  mutate(center_q1= mean(q1),
         ...
         center_q5= mean(q5)) %>% 
  ungroup() %>% 
  select(GCAM_region_ID, center_q1,center_q2,center_q3,center_q4,
         center_q5,year) %>% 
  distinct()->centers 
...
data_comp_PC %>% left_join(pc_raw %>% select(GCAM_region_ID,year, q1,q2,q3,q4,q5)
               ...
               left_join(centers, by= c("GCAM_region_ID","year")) %>% 
...
g <- ggplot(data= data_comp_PC, aes(x=Component1))+
...
df_temp %>% left_join(centers %>% filter(year ==2015) %>% select(-year), by= c("GCAM_region_ID")) %>% 
                 mutate(q1= ((Component1* pc_loadings[1,1])+center_q1),
                      q2= ((Component1* pc_loadings[2,1])+center_q2),
                      q3= ((Component1* pc_loadings[3,1])+center_q3),
                      q4= ((Component1* pc_loadings[4,1])+center_q4),
                      q5= ((Component1* pc_loadings[5,1])+center_q5),
...
df_temp_rao %>% 
  left_join(centers, by= c("GCAM_region_ID")) %>% 
  mutate(q1= ((Component1* pc_loadings[1,1])+center_q1),
         q2= ((Component1* pc_loadings[2,1])+center_q2),
         q3= ((Component1* pc_loadings[3,1])+center_q3),
         q4= ((Component1* pc_loadings[4,1])+center_q4),
...
g <- ggplot(data= GDP_data_forecast_SSPs   %>% bind_rows(historical_data) %>% mutate(shares= `Income (net)`) %>% left_join(GCAM_regions, by = c("GCAM_region_ID"))%>% 
              filter(Category=="q1", region %in% c("South Africa")), aes(x=year, y=shares, color= category))+
  ...
  ggtitle("Income distribution shares for Q1 for GCAM regions (Autoregressive + GDP model)")+
...
g <- ggplot(data= GDP_data_forecast_SSPs  %>% bind_rows(historical_data)  %>%   mutate(shares= `Income (net)`*100) %>% left_join(GCAM_regions, by = c("GCAM_region_ID"))%>% 
                filter(Category=="q5") %>% filter(region %in% c("India","China","USA"))%>% mutate(id = paste0(GCAM_region_ID,sce)) %>% distinct(), aes(x=year, y=gini, group=id, color=sce))+
...
g <- ggplot(data= Wider_data_interpolated %>%   filter(GCAM_region_ID==15) %>% 
              mutate(country = if_else(country == "Bosnia and Herzegovina","Bosnia",if_else(
                country == "Macedonia, the former Yugoslav Republic of","Macedonia",country
              ))), aes(x=year, y=Income..net.))+
  geom_line(aes(color= Category),size=1.2)+
  facet_wrap(~country)+
  ylab("Income shares as a percentage of total income")+
  ggtitle("Income distribution shares for Eastern Europe members from raw WIDER data")+
  scale_color_npg()
...
write.csv(GDP_data, "Net_income_quintiles_GCAM_regions_1967_2100_27Sep2021.csv")            
