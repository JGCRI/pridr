...
compute_lognormal_country <- function(df){
  ...
  gini <- unique(df$gini)
  max_income <- (df$gdp_ppp_pc_usd2011)*14
  len_sample <- 2000
  year <- unique(df$year)
...
compute_PC_model_components <- function(df,final_year=2015){
  ...
  for (row in 1:nrow(df)){
    if(df$year[row]<final_year){
      ...
      df %>%
        ...
        adjust_negative_predicted_features(grouping_variables = c("country","year"))->computed_deciles

      palma_ratio <- compute_palma_ratio(computed_deciles %>% filter(year == df$year[row]),income_col = "pred_shares")



      computed_deciles %>%
        filter(year == df$year[row]) %>%
        filter(Category=="d9")->ninth_decile_value
      print(unique(palma_ratio$palma_ratio))

      df$lagged_ninth_decile[row+1] <- unique(ninth_decile_value$pred_shares)
